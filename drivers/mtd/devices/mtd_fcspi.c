...
static int fcspi_write_work(struct fcspi_message *msg)
{
	...
	if ((msg->flash_offset & 3) || (msg->len & 3)
			|| (u32)(msg->buf) & (3 | ((1 << ctl->paeshift) - 1))) {
...
/**
 * get_quatro_extaddrmode - returns whether we are in extended
...
static int __init get_paeshift(void)
{
	...
	mapped_address = ioremap(QUATRO_DDRMEMCTRL, 0x100);
	if (mapped_address) {
		if (readl((void *)mapped_address+QUATRO_EXTENDED_ADDRESS_MODE_OFF) & QUATRO_EXTENDED_ADDRESS_MODE__EAM__MASK) {
			extaddrmode = 4;
			printk(KERN_INFO "Quatro in extended addressing mode.\n");
		}
		else {
			extaddrmode = 0;
			printk(KERN_INFO "Quatro in 32-bit addressing mode.\n");
		}
